# ZotQuery 3.0.11 隐私清理候选版安装与验收

此包尚有 [发布审计](RELEASE-AUDIT-ZH.md) 中的门禁与本地接口缺口，仅供隔离环境验收，暂不建议替换正在使用的正式安装。

1. 备份 Zotero 数据目录，并正常退出 Zotero。
2. 通过 Zotero 插件管理器安装 `ZotQuery-3.0.11-public-candidate.xpi`，不要直接覆盖已安装的 XPI 文件；同一扩展 ID 会作为升级处理。若仍失败，请保留完整错误文字或截图，不要覆盖 profile/extension 文件。
