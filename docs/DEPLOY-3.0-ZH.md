# ZotQuery 3.0.13 隐私清理候选版安装与验收

此包尚有 [发布审计](RELEASE-AUDIT-ZH.md) 中的门禁与本地接口缺口，仅供隔离环境验收，暂不建议替换正在使用的正式安装。

1. 备份 Zotero 数据目录，并正常退出 Zotero。
2. 通过 Zotero 插件管理器安装 `ZotQuery-3.0.13-source-candidate.xpi`，不要直接覆盖已安装的 XPI 文件。此版使用独立扩展 ID，不会自动升级使用旧 ID 的 3.0.11 候选包，也不会自动迁移旧索引或设置。先备份 profile；需要移除旧候选包时通过插件管理器操作，勿删除原有数据库。若安装失败，保留完整错误文字或截图。
