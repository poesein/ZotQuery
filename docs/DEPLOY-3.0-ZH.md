...
2. 通过 Zotero 插件管理器安装 `ZotQuery-3.0.14-source-candidate.xpi`，不要直接覆盖已安装的 XPI 文件。此版沿用 3.0.12/3.0.13 的独立扩展 ID，可原位升级该分支；但不会自动升级使用旧 ID 的 3.0.11 候选包，也不会自动迁移旧 ID 索引或设置。先备份 profile；需要移除旧候选包时通过插件管理器操作，勿删除原有数据库。若安装失败，保留完整错误文字或截图。
