...
状态：**3.0.12 候选源码已修补下列四项，但尚未经过 Zotero 10 实机安装与迁移验收，不建议作为已验收正式版公开发布**。原始审阅对象为 3.0.9；修补位于独立候选源码，未修改正式安装。`node tests/regression-final.mjs` 包含新反例并通过；未运行破坏性迁移或索引重建。

3.0.12 另将扩展 ID 改为 `zotquery@poesein.github.io`，并隔离偏好、chrome 资源及 PDF/Note/Research 数据库。它不会原位升级旧 ID 的 3.0.11 候选包，也不会自动复制旧数据；旧版仍需通过 Zotero 插件管理器识别和处理，实机并装与迁移尚待验证。
