...
状态：**3.0.11 候选源码已修补下列四项，但尚未经过 Zotero 10 实机安装与迁移验收，不建议作为已验收正式版公开发布**。原始审阅对象为 3.0.9；修补位于独立候选源码，未修改正式安装。`node tests/regression-final.mjs` 包含新反例并通过；未运行破坏性迁移或索引重建。
