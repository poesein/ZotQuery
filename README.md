...
**Release status:** 3.0.13 is a privacy-sanitized [**public pre-release candidate**](https://github.com/poesein/ZotQuery/releases/tag/v3.0.13), not an accepted production release. Offline regression tests are available, but this candidate has not passed live Zotero 10 startup, upgrade, indexing, and MCP acceptance tests. The bundled ONNX model is tracked with Git LFS; `git lfs pull` is needed after cloning the source. Read the [known limitations](#limitations-and-release-status) and [redistribution checks](#attribution-licenses-and-privacy) before installing.
...
1. Confirm **Zotero 10.0.x**, download the candidate XPI from the [v3.0.13 pre-release](https://github.com/poesein/ZotQuery/releases/tag/v3.0.13), and review the [release audit](docs/RELEASE-AUDIT-ZH.md). Back up the Zotero data directory and close Zotero before an upgrade. Test the candidate first in a separate or backed-up profile.
2. Install the candidate XPI through Zotero's plugin manager and restart. ZotQuery now uses its own extension ID (`zotquery@poesein.github.io`), preferences, chrome resources, and SQLite files, so it can coexist with ZotSeek without sharing writable state. This ID change means 3.0.13 is **not an in-place upgrade** of the older ZotQuery 3.0.11 candidate, which used ZotSeek's ID. Old indexes and settings remain untouched and are not automatically copied; back up your profile and reindex or plan a separate, reviewed migration. Do not manually replace extension files, databases, or preferences.
...
4. Choose PDF scope (`title/abstract` or `full PDF`), library scope, and the active embedding model. For source-level evidence work, full-PDF indexing is needed; title/abstract indexing cannot provide a verified PDF passage. An OpenAI-compatible embedding server can run locally or at a trusted private IPv4 address (for example `http://192.168.1.10:11434`); public Internet hosts are rejected. Verify `/v1/models` and `/v1/embeddings` from the Zotero machine, and note that plain HTTP exposes texts and API keys on the LAN. Server-model IDs include the endpoint so vectors from different hosts are not silently mixed; switching hosts starts a separate cache and requires indexing. Test a small mixed PDF/Note sample before a large run.
