...
**Release status:** 3.0.14 is a privacy-sanitized [**public pre-release candidate**](https://github.com/poesein/ZotQuery/releases/tag/v3.0.14), not a production release. Its local installation was confirmed in Zotero 10: the extension manager and live health both reported 3.0.14, existing PDF/Note indexes remained available, and an authenticated MCP search succeeded. Switching between two different live hosts has not been accepted end-to-end because the old loopback server was offline during validation. The bundled ONNX model is tracked with Git LFS; `git lfs pull` is needed after cloning the source. Read the [known limitations](#limitations-and-release-status) and [redistribution checks](#attribution-licenses-and-privacy) before installing.
...
1. Confirm **Zotero 10.0.x**, download the candidate XPI from the [v3.0.14 pre-release](https://github.com/poesein/ZotQuery/releases/tag/v3.0.14), and review the [release audit](docs/RELEASE-AUDIT-ZH.md). Back up the Zotero data directory and close Zotero before an upgrade. Test the candidate first in a separate or backed-up profile.
2. Install the candidate XPI through Zotero's plugin manager and restart. ZotQuery uses its own extension ID (`zotquery@poesein.github.io`), preferences, chrome resources, and SQLite files, so it can coexist with ZotSeek without sharing writable state. This ID change means 3.0.14 is **not an in-place upgrade** of the older ZotQuery 3.0.11 candidate, which used ZotSeek's ID. It does upgrade the isolated-ID 3.0.12/3.0.13 line. Old-ID indexes and settings are not automatically copied; back up your profile and plan a separate, reviewed migration. Do not manually replace extension files, databases, or preferences.
...
4. Choose PDF scope (`title/abstract` or `full PDF`), library scope, and the active embedding model. For source-level evidence work, full-PDF indexing is needed; title/abstract indexing cannot provide a verified PDF passage. An OpenAI-compatible embedding server can run locally or at a trusted private IPv4 address (for example `http://192.168.1.10:11434`); public Internet hosts are rejected. Verify `/v1/models` and `/v1/embeddings` from the Zotero machine, and note that plain HTTP exposes texts and API keys on the LAN. When changing an Ollama endpoint, ZotQuery reuses PDF/Note vectors only after checking the model digest, dimensions, preprocessing, and fixed embedding probes; otherwise the old index remains intact and the new endpoint needs its own index. Other OpenAI-compatible servers without verifiable Ollama metadata keep separate cache identities. See [endpoint compatibility](docs/MODEL-ENDPOINT-COMPATIBILITY.md). Test a small mixed PDF/Note sample before a large run.
...
- **Partial live acceptance:** one Zotero 10 profile passed installed-version, health, existing-index, live embedding, and authenticated MCP checks. This is not a fresh-profile test, a full migration test, or a two-live-host switching test. Do not present this candidate as a production release.
